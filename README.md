...
| `mainProgram` | The name of the binary to set as `meta.mainProgram`. When not set, it is auto-detected from the `[[bin]]` sections in `Cargo.toml` (falling back to `package.name` when the default `src/main.rs` is used). Overriding this is useful when the binary name does not match the package name. |
